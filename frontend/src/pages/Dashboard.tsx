...
export function Dashboard({ reports, studies, measurements, auditEvents, patientName, onNavigate, onOpenReport }: DashboardProps) {
  const recentReports = [...reports]
    .sort((a, b) => (b.uploadedAt ?? b.date).localeCompare(a.uploadedAt ?? a.date))
    .slice(0, 5);
  ...
  const hba1cData = measurements
    ...
    .map((m) => ({ date: m.reportDate, value: m.value, unit: m.unit }));
  const hba1cUnit = hba1cData[hba1cData.length - 1]?.unit ?? '';

  const processingReports = reports.filter((r) => isProcessing(r.status));
  const reviewReports = reports.filter((r) => isReviewable(r.status));
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Card hover onClick={() => onNavigate('reports')}>
          <div className="p-5">
            ...
            <p className="mt-3 text-2xl font-bold text-neutral-900 dark:text-white" data-testid="kpi-reports">{reports.length}</p>
        ...
        <Card hover onClick={() => onNavigate('imaging')}>
          <div className="p-5">
            ...
            <p className="mt-3 text-2xl font-bold text-neutral-900 dark:text-white" data-testid="kpi-studies">{studies.length}</p>
        ...
        <Card hover onClick={() => onNavigate('reports')}>
          <div className="p-5">
            ...
            <p className="mt-3 text-2xl font-bold text-neutral-900 dark:text-white" data-testid="kpi-insights">{aiInsights}</p>
            <p className="text-xs text-neutral-500 dark:text-neutral-400">AI Report Summaries</p>
          </div>
        </Card>
      </div>

      {/* Attention items */}
      {(processingReports.length > 0 || reviewReports.length > 0 || failedReports.length > 0) && (
        <Card className="border-amber-200 bg-amber-50/50 dark:border-amber-700/30 dark:bg-amber-900/5">
          <div className="flex items-center gap-3 p-4">
            <AlertCircle className="h-5 w-5 shrink-0 text-amber-600 dark:text-amber-400" />
            <div className="flex-1">
              <p className="text-sm font-medium text-amber-800 dark:text-amber-300" data-testid="dashboard-attention">
                {[
                  processingReports.length && `${processingReports.length} processing`,
                  reviewReports.length && `${reviewReports.length} awaiting your review`,
                  failedReports.length && `${failedReports.length} could not be processed`,
                ].filter(Boolean).join(' · ')}
              </p>
              <p className="text-xs text-amber-600 dark:text-amber-400/80">
                {[...processingReports, ...reviewReports, ...failedReports].map((r) => r.title).join(', ')}
              </p>
            </div>
        ...
        <Card>
          ...
          <div className="divide-y divide-neutral-100 dark:divide-neutral-800">
            {recentReports.length === 0 && (
              <p className="px-5 py-8 text-center text-sm text-neutral-400">No reports yet. Upload one from the Reports workspace.</p>
            )}
      ...
      <Card>
        <CardHeader
          title="Health Trends"
          subtitle="HbA1c results you have confirmed"
        ...
        <div className="px-5 pb-5" style={{ height: 220 }}>
          {hba1cData.length === 0 ? (
            <p className="py-16 text-center text-sm text-neutral-400">No confirmed HbA1c results yet.</p>
          ) : (
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={hba1cData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <YAxis tick={{ fontSize: 11 }} stroke="currentColor" className="text-neutral-400" domain={['auto', 'auto']} />
              <Tooltip
                contentStyle={{ borderRadius: 8, border: '1px solid', fontSize: 12, background: 'var(--tooltip-bg, #fff)' }}
                formatter={(value) => [`${value ?? '-'} ${hba1cUnit}`, 'HbA1c']}
              />
