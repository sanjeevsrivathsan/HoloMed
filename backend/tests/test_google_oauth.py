...
# ── same-tab navigation (no popup, no extra window) ────────────────────
def redirect_query(resp):
    location = resp.headers["location"]
    assert location.startswith(POST_LOGIN)
    return {k: v[0] for k, v in parse_qs(urlsplit(location).query).items()}


def test_sign_in_runs_in_the_current_tab_and_sets_only_the_flow_cookie(client, google):
    """The browser navigates to /api/v1/auth/google itself; nothing marks or needs a second window."""
    resp, p = start(client, google)
    assert resp.status_code == 302 and resp.headers["location"].startswith(google_oauth.AUTHORIZATION_ENDPOINT)
    assert [c.split("=")[0] for c in set_cookies(resp)] == [google_oauth.FLOW_COOKIE]
    assert p["code_challenge_method"] == "S256" and len(p["state"]) >= 43 and len(p["nonce"]) >= 43

    resp = callback(client, code=AUTH_CODE, state=p["state"])
    # success lands on the plain post-login URL: no auth parameters, no popup marker, no tokens
    assert resp.headers["location"] == POST_LOGIN
    assert session_cookie_set(resp) and flow_cookie_cleared(resp)
    assert client.get("/api/v1/auth/me").status_code == 200


def test_popup_query_parameter_is_not_honoured(client, google):
    """A leftover ?popup=1 link must not change the redirect target or set extra cookies."""
    resp, p = start(client, google, "/api/v1/auth/google?popup=1")
    assert all(c.split("=")[0] == google_oauth.FLOW_COOKIE for c in set_cookies(resp))
    assert redirect_query(callback(client, code=AUTH_CODE, state=p["state"])) == {}


def test_failures_return_to_the_app_with_a_short_code(client, google):
    _, p = start(client, google)
    assert redirect_query(callback(client, error="access_denied", state=p["state"])) ==         {"auth_error": "google_cancelled"}
    _, p = start(client, google)
    assert redirect_query(callback(client, code=AUTH_CODE, state="attacker")) == {"auth_error": "invalid_state"}
    assert users() == []
    assert redirect_query(client.get("/api/v1/auth/google/link")) == {"auth_error": "link_requires_login"}


def test_link_returns_to_the_app_with_a_notice(client, google):
    with Session(engine) as s:
        create_user(EMAIL, "Password123!", s)
    login_password(client, EMAIL, "Password123!")
    _, p = start(client, google, "/api/v1/auth/google/link")
    assert redirect_query(callback(client, code=AUTH_CODE, state=p["state"])) == {"auth_notice": "google_linked"}
